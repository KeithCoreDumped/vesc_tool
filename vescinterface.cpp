...
QVariantList VescInterface::listSerialPorts()
{
	...
	foreach(const QSerialPortInfo & port, ports) {
		...
		else if (port.productIdentifier() == 0x1001) {
			info.name.insert(0, "ESP32 - ");
			info.isEsp = true;
			index = 0;
		}
		else {
			//info.name.insert(0, "UNKNOWN - ");
			info.name = port.description();
			info.isVesc = false;
		}

		// trim text
		const int max_length = 23;
		if (info.name.length() > max_length) {
			info.name = info.name.mid(0, max_length - 3) + "...";
		}

		res.insert(index, QVariant::fromValue(info));
	}
#endif
	// make Serial STM32 on top of the list, and make qml happy
